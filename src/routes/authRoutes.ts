...
router.post("/login/verify", async (req: Request, res: Response) => {
  ...
  await findOneAndUpdateQuery(
    User,
    { _id: userId, fingerPrint: fpId },
    { isLoggedIn: true }
  );
  return res
    .cookie("cookie-id", token, {
      sameSite: "none",
      domain: `${keys.clientUrl}`,
    })
    .status(200)
...
router.post("/login", async (req: Request, res: Response) => {
  try {
    ...
    let user = (await findOneQuery(User, {
      email,
      fingerPrint,
    })) as unknown as IUser;

    if (!user) {
      user = (await createDocumentQuery(User, {
        email,
        fingerPrint,
      })) as unknown as IUser;
    }
    const token = jwt.sign(
      { userId: user._id, fpId: fingerPrint },
      keys.cookieKey
    );
...
router.post(
  ...
  auth,
  async (req: Request, res: Response): Promise<void | Message> => {
    try {
